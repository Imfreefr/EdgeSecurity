const API_BASE = (() => {
    const configured = window.EDGE_API_BASE || document.querySelector('meta[name="edge-api-base"]')?.content;
    if (configured) {
        const base = configured.replace(/\/$/, '');
        return base.endsWith('/api') ? base : `${base}/api`;
    }

    // Through Cloudflare Tunnel (or any reverse proxy), frontend and API share the same origin.
