...
function currentUser(){ const s=EdgeAuth.current();
return s ? EdgeDB.users.find(u=>u.id===s.id) : null;
}
function canUseCamera(){ const u=currentUser();
return !!u && (u.cargo==='administrador' || u.permissoes?.usar_camera_dispositivo !== false);
}
function canRegisterCamera(){ const u=currentUser();
return !!u && (u.cargo==='administrador' || u.permissoes?.gerenciar_cameras === true);
}
...
if(!items.length){ registeredList.innerHTML='<div class="empty-state">Nenhuma câmera cadastrada.</div>';
return;
}
registeredList.innerHTML=items.map(c=>{
const isIp=['ip','rtsp','wifi'].includes(c.tipo);
const action=isIp ? `<button class="btn btn-secondary btn-small" data-ipview="${escapeHtml(c.id)}">Visualizar</button>` : (c.device_id ? `<button class="btn btn-secondary btn-small" data-registered="${escapeHtml(c.id)}">Usar</button>` : '');
...
registeredList.querySelectorAll('[data-registered]').forEach(btn=>btn.onclick=()=>{ const c=registeredCameras().find(x=>x.id===btn.dataset.registered);
if(c?.device_id){ select.value=c.device_id;
startCamera(c.device_id);
} });
registeredList.querySelectorAll('[data-ipview]').forEach(btn=>btn.onclick=()=>startIpCamera(btn.dataset.ipview));
registeredList.querySelectorAll('[data-delete-camera]').forEach(btn=>btn.onclick=async()=>{ if(!confirm('Remover esta câmera do cadastro?')) return;
try{await EdgeAPI.del('/cameras/'+btn.dataset.deleteCamera);
await EdgeData.load();
renderRegistered();
showToast('Câmera removida.')}catch(e){showToast(e.message)}});
}
function setIpError(msg=''){ if(!ipError)return;
ipError.textContent=msg;
ipError.hidden=!msg;
}
async function testIpCamera(){
const url=ipUrl?.value.trim();
if(!url){setIpError('Informe o endereço RTSP ou HTTP da câmera.');
return;
}
try{ testIp.disabled=true;
...
showToast('Stream da câmera acessível.');
}catch(e){testedIp=false;
saveIp.disabled=true;
ipStatus.textContent='Falha na conexão';
ipStatus.classList.remove('live');
setIpError(e.message);
showToast(e.message)}
finally{testIp.disabled=false;
testIp.textContent='Testar conexão'}
}
async function saveIpCamera(){
if(!testedIp){showToast('Teste a conexão antes de cadastrar.');
return;
}
if(!canRegisterCamera()){showToast('Você não possui permissão para cadastrar câmeras.');
return;
}
try{await EdgeAPI.post('/cameras',{nome:ipName.value.trim()||'Câmera IP',tipo:'ip',endereco:ipUrl.value.trim(),localizacao:ipLocation.value.trim()||null,status:'ativo'});
await EdgeData.load();
renderRegistered();
ipName.value='';
ipUrl.value='';
...
ipStatus.classList.remove('live');
showToast('Câmera IP cadastrada.')}catch(e){showToast(e.message)}
}
...
if(!token){showToast('Sessão inválida.');
return;
}
const img=document.createElement('img');
img.id='ip-stream';
img.alt='Stream da câmera IP';
img.className='ip-stream';
img.onerror=()=>{setCameraError('Não foi possível receber o stream. Verifique a conexão da câmera e o endereço RTSP/HTTP.');
...
const u=currentUser();
if(!u){showToast('Sessão inválida.');
return;
}
if(!canRegisterCamera()){showToast('Você não possui permissão para cadastrar câmeras.');
return;
...
if(!d){showToast('Detecte e selecione uma câmera antes de cadastrar.');
return;
}
const existing=registeredCameras().find(c=>c.device_id===d.deviceId);
if(existing){showToast('Esta câmera já está cadastrada.');
return;
}
try { await EdgeAPI.post('/cameras',{nome:d.label||'Câmera do dispositivo',tipo:'browser',device_id:d.deviceId,status:'ativo'});
await EdgeData.load();
renderRegistered();
...
function setStatus(text, live = false) { status.textContent = text;
status.classList.toggle('live', live);
}
function setAIStatus(text, live = false) { aiStatus.textContent = text;
aiStatus.classList.toggle('live', live);
}
function drawResults(data) {
const automaticAlert = data.alert;
if (automaticAlert) {
showToast(`⚠️ ${automaticAlert.nivel}: ${automaticAlert.descricao}`);
if ('Notification' in window && Notification.permission === 'granted') {
new Notification(`EdgeSecurity — ${automaticAlert.nivel}`, { body: automaticAlert.descricao });
}
}
...
ctx.lineWidth = line;
ctx.strokeStyle = d.class_name === 'human' ? '#22c55e' : ['machine','forklift'].includes(d.class_name) ? '#f59e0b' : '#60a5fa';
ctx.strokeRect(x1,y1,x2-x1,y2-y1);
ctx.font = 'bold 16px Inter, sans-serif';
const text = `${d.label} ${(d.confidence*100).toFixed(0)}%${d.track_id != null ? ` #${d.track_id}` : ''}`;
ctx.fillStyle = ctx.strokeStyle;
ctx.fillRect(x1, Math.max(0,y1-24), ctx.measureText(text).width+10, 24);
ctx.fillStyle = '#08111f';
ctx.fillText(text, x1+5, Math.max(17,y1-6));
});
humanCount.textContent = (data.detections || []).filter(d=>d.class_name==='human').length;
machineCount.textContent = (data.detections || []).filter(d=>['machine','forklift'].includes(d.class_name)).length;
...
if (!stream) { showToast('Inicie uma câmera antes da análise.');
return;
}
const registered = registeredCameras().find(c=>c.device_id===select.value);
if(!registered){ showToast('Cadastre esta câmera antes de iniciar a análise.');
return;
}
const cameraId = registered.id;
setAIStatus('Conectando à IA…');
EdgeAI.connect(cameraId, drawResults, msg => { stopAIAnalysis();
showToast(msg);
setAIStatus('Erro na IA');
}, () => {
aiRunning = true;
setAIStatus('YOLO26 AO VIVO', true);
startAI.disabled=true;
...
function stopCamera() {
stopAIAnalysis();
if(window._ipStream){window._ipStream.src='';
...
setCameraError('');
if (!CameraAPI.supported()) { setCameraError(CameraAPI.supportError());
showToast('Acesse o sistema por localhost ou HTTPS.');
return;
}
if (!canUseCamera()) { setCameraError('Sua conta não possui permissão para usar a câmera deste dispositivo.');
return;
}
try {
stopAIAnalysis();
...
setStatus('AO VIVO',true);
const track=stream.getVideoTracks()[0], currentId=track.getSettings().deviceId, current=devices.find(d=>d.deviceId===currentId);
...
devices.forEach((d,i)=>{const o=document.createElement('option');
o.value=d.deviceId;
...
list.querySelectorAll('[data-device]').forEach(btn=>btn.onclick=()=>{select.value=btn.dataset.device;
startCamera(btn.dataset.device);
});
} catch(err){
if(err?.name==='UNSUPPORTED'){ setCameraError(CameraAPI.supportError());
list.innerHTML='<div class="empty-state">O navegador só disponibiliza câmeras em localhost ou HTTPS.</div>';
}
...
setCameraError('');
if(!CameraAPI.supported()){ setCameraError(CameraAPI.supportError());
showToast('Abra o EdgeSecurity por localhost ou HTTPS.');
return;
}
if(!canUseCamera()){ setCameraError('Sua conta não possui permissão para usar a câmera deste dispositivo.');
return;
...
function initializeCameraPage(){ renderRegistered();
register.disabled=true;
if(!canUseCamera()){ start.disabled=true;
detect.disabled=true;
setCameraError('Sua conta não possui permissão para usar a câmera deste dispositivo.');
}
