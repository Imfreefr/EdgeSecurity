...
(function(){
 ...
 const nav=[
  ...
  ["atividades","⌁","Monitor de Atividades"],
  ["configuracoes","⚙","Configurações"]
 ];
 if(s.cargo==="administrador") nav.splice(2,0,["usuarios","♙","Usuários e Permissões"]);
 document.getElementById("app").innerHTML=`<div class="app-shell">
 <aside class="sidebar"><div class="sb-logo">Edge<span>Security</span></div><nav class="sb-nav">${nav.map(n=>`<button class="sb-item ${page===n[0]?"active":""}" data-go="${n[0]}"><span class="ico">${n[1]}</span>${n[2]}</button>`).join("")}</nav>
 <div class="sb-user"><strong>${s.nome}</strong>${s.cargo==="administrador"?"Administrador":"Usuário"}</div><button class="sb-logout" id="logout">↪ Sair</button></aside>
