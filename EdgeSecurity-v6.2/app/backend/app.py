...
app = FastAPI(title='EdgeSecurity API', version='0.6.2')
...
def login(data: LoginIn):
    ...
    with conn() as db:
        ...
        now_ts = time.time()
        SESSIONS[token] = {'id': row['id'], 'nome': row['nome'], 'email': row['email'], 'cargo': row['cargo'], 'login_at': now_ts, 'last_recorded': now_ts}
...
def logout(authorization: str | None = Header(default=None)):
    ...
    with conn() as db:
        ...
        db.execute('UPDATE usuarios SET ultimo_logout=? WHERE id=?', (now(), session['id']))
...
def test_camera(data: CameraTestIn, authorization: str | None = Header(default=None)):
    ...
    cap = open_ip_camera(data.endereco)
    if cap is None:
    ...
    if not ok or frame is None:
        raise HTTPException(422, 'A câmera respondeu, mas não forneceu um quadro de vídeo.')
    return {'ok': True, 'width': int(frame.shape[1]), 'height': int(frame.shape[0]), 'message': 'Stream acessível.'}
...
def camera_mjpeg(cid: str, token: str):
    ...
    def frames():
        try:
            ...
            cap.release()
    return StreamingResponse(frames(), media_type='multipart/x-mixed-replace; boundary=frame', headers={'Cache-Control':'no-cache','Pragma':'no-cache'})
