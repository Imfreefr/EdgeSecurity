(() => {
  ...
  function renderRegistered(){
    ...
    registeredList.querySelectorAll('[data-delete-camera]').forEach(btn=>btn.onclick=async()=>{ if(!confirm('Remover esta câmera do cadastro?')) return; try{await EdgeAPI.del('/cameras/'+btn.dataset.deleteCamera);await EdgeData.load();renderRegistered();showToast('Câmera removida.')}catch(e){showToast(e.message)}});
  }

  function setIpError(msg=''){ if(!ipError)return; ipError.textContent=msg; ipError.hidden=!msg; }
  async function testIpCamera(){
    const url=ipUrl?.value.trim(); if(!url){setIpError('Informe o endereço RTSP ou HTTP da câmera.');return;}
    try{ testIp.disabled=true; testIp.textContent='Testando…'; setIpError(''); ipStatus.textContent='Testando…'; ipStatus.classList.remove('live');
      await EdgeAPI.post('/cameras/test',{endereco:url}); testedIp=true; saveIp.disabled=!canRegisterCamera(); ipStatus.textContent='Conexão OK'; ipStatus.classList.add('live'); showToast('Stream da câmera acessível.');
    }catch(e){testedIp=false;saveIp.disabled=true;ipStatus.textContent='Falha na conexão';ipStatus.classList.remove('live');setIpError(e.message);showToast(e.message)}
    finally{testIp.disabled=false;testIp.textContent='Testar conexão'}
  }
  async function saveIpCamera(){
    if(!testedIp){showToast('Teste a conexão antes de cadastrar.');return;}
    if(!canRegisterCamera()){showToast('Você não possui permissão para cadastrar câmeras.');return;}
    try{await EdgeAPI.post('/cameras',{nome:ipName.value.trim()||'Câmera IP',tipo:'ip',endereco:ipUrl.value.trim(),localizacao:ipLocation.value.trim()||null,status:'ativo'});await EdgeData.load();renderRegistered();ipName.value='';ipUrl.value='';ipLocation.value='';testedIp=false;saveIp.disabled=true;ipStatus.textContent='Não testada';ipStatus.classList.remove('live');showToast('Câmera IP cadastrada.')}catch(e){showToast(e.message)}
  }
  function startIpCamera(cid){
    const c=registeredCameras().find(x=>x.id===cid); if(!c)return;
    stopCamera();
    const token=EdgeAPI.token();
    if(!token){showToast('Sessão inválida.');return;}
    const img=document.createElement('img'); img.id='ip-stream'; img.alt='Stream da câmera IP'; img.className='ip-stream';
    img.onerror=()=>{setCameraError('Não foi possível receber o stream. Verifique a conexão da câmera e o endereço RTSP/HTTP.');setStatus('Erro no stream');};
    img.onload=()=>{empty.style.display='none';video.style.display='none';canvas.style.display='none';setStatus('AO VIVO',true);label.textContent=c.nome;stop.disabled=false;startAI.disabled=true;};
  ...
  function stopCamera() {
    stopAIAnalysis(); if(window._ipStream){window._ipStream.src='';window._ipStream.remove();window._ipStream=null;}
