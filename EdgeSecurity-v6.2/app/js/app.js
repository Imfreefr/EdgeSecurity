(function(){
 ...
 const setCollapsed=(value)=>{if(!desktopQuery.matches)return; sidebar.classList.toggle('collapsed',value);};
 const bindSidebar=()=>{
   if(desktopQuery.matches){
     sidebar.classList.add('collapsed');
     sidebar.onmouseenter=()=>setCollapsed(false);
     sidebar.onmouseleave=()=>setCollapsed(true);
   }else{
 ...
 bindSidebar();
 desktopQuery.addEventListener?.('change',bindSidebar);
 ...
 const heartbeat=async()=>{try{await EdgeAPI.post('/auth/heartbeat',{});}catch(_){} };
 heartbeat();
