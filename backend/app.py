...
def init_db():
    ...
    with conn() as db:
        ...
        usuario_cols = {r['name'] for r in db.execute('PRAGMA table_info(usuarios)').fetchall()}
        if 'administrador_primario' not in usuario_cols:
            db.execute('ALTER TABLE usuarios ADD COLUMN administrador_primario INTEGER NOT NULL DEFAULT 0')
...
def user_dict(row, db) -> dict[str, Any]:
    ...
    return {
        **dict(row), 'senha': None, 'permissoes': permissions, 'cameras': cams,
        'administrador_primario': bool(row['administrador_primario']),
    }


def get_session_for_token(token: str) -> dict[str, Any] | None:
    """Return the session for a token, expiring (and dropping) it if idle
    for longer than SESSION_IDLE_TIMEOUT. Renews the idle timer on hit."""
...
def create_ai_alert(camera_id, risk):
    ...
    with conn() as db:
        ...
        db.execute('INSERT INTO alertas(id,camera_id,tipo,nivel,descricao,status,data_hora) VALUES(?,?,?,?,?,?,?)', (
            alert_id, camera_id, 'IA - Aproximação',
            'Crítico' if risk.get('level') == 'critical' else 'Alto',
            description, 'Aberto', now()
        ))
    return alert_id
...
def setup(data: SetupIn):
    with conn() as db:
        ...
        db.execute('INSERT INTO usuarios(id,nome,email,senha_hash,cargo,status,criado_em,administrador_primario) VALUES(?,?,?,?,?,?,?,1)', (uid, data.nome.strip(), data.email.strip().lower(), hash_password(data.senha), 'administrador', 'ativo', now()))
...
def _check_login_rate_limit(key: str) -> None:
    """Basic in-memory brute-force guard: LOGIN_MAX_ATTEMPTS failed logins per
...
def login(data: LoginIn, request: Request):
    ...
    with conn() as db:
        ...
        SESSIONS[token] = {'id': row['id'], 'nome': row['nome'], 'email': row['email'], 'cargo': row['cargo'], 'login_at': now_ts, 'last_recorded': now_ts, 'last_seen': now_ts, 'session_id': session_id}
        stamp = now()
    ...
    return {'token': token, 'user': {'id': row['id'], 'nome': row['nome'], 'email': row['email'], 'cargo': row['cargo'], 'administrador_primario': bool(row['administrador_primario'])}}
...
def delete_user(uid: str, authorization: str | None = Header(default=None)):
    ...
    with conn() as db:
        ...
        activity(db, admin['id'], 'exclusão de usuário', f'Usuário {row["email"]} excluído permanentemente')
