async function initActivities() {
  try {
    const [data, users] = await Promise.all([
      EdgeAPI.get("/activities"),
      EdgeAPI.get("/monitor/usuarios"),
    ]);
    ...
    document.getElementById("activity-stats").innerHTML = [
      [
        ...
        users.length,
        users.length ? "Total no sistema" : "Nenhum usuário ainda",
      ],
      [
        "Usuários online",
        online,
        online ? `${online} conectado(s) agora` : "Ninguém online no momento",
      ],
      [
        "Ações registradas",
        data.length,
        data.length ? "Histórico de uso" : "Nenhuma ação registrada",
      ],
      [
        "Tempo total online",
        formatDuration(
          users.reduce((n, u) => n + Number(u.tempo_total_ativo || 0), 0),
        ),
        "Tempo somado de uso",
      ],
    ]
